...
token_type Commands::determineToken(char sep, token_type cmdType)
{
    ...
    else if (isspace(sep))
    {
        if (cmdType == JOIN_CMD)
            ...
            _paramCounter == 1 ? tokenType = CHANNEL : (_paramCounter == 2 ? tokenType = MODE_STR : tokenType = MODE_ARG);
        else if (cmdType == PART_CMD)
            _paramCounter == 1 ? tokenType = CHANNEL : (_paramCounter == 2 ? tokenType = REASON : tokenType = NONE);
        else if (cmdType == KICK_CMD)
            _paramCounter == 1 ? tokenType = CHANNEL : (_paramCounter == 2 ? tokenType = NICK : (_paramCounter == 3 ? tokenType = COMMENT : tokenType = NONE));
        else if (cmdType == TOPIC_CMD)
            _paramCounter == 1 ? tokenType = CHANNEL : (_paramCounter == 2 ? tokenType = TOPIC_MSG : tokenType = NONE);
        else if (cmdType == INVITE_CMD)
...
std::pair<std::string, std::vector<std::string> > Commands::getNextParam(OPTION option)
{
    ...
    {
        if ((*it).type != tokenType && (*it).type != COMMA)
        {
            tokenType = (*it).type;
            break;
        }
        if ((it)->type != COMMA)
            paramData.second.push_back((*it).data);
        it++;
    }

    if (!paramData.second.empty())
...
bool Commands::isEnoughParam(token_type const &cmd)
{
    ...
    if (cmd == JOIN_CMD && _paramCounter < 2)
        ...
        return false;
    else if (cmd == PART_CMD && _paramCounter < 2)
        return false;
    else if (cmd == KICK_CMD && _paramCounter < 3)
        return false;
    else if (cmd == TOPIC_CMD && _paramCounter < 2)
        return false;
    else if (cmd == WHOIS_CMD && _paramCounter < 2)
        return false;
    else if (cmd == INVITE_CMD && _paramCounter < 3)
        return false;
    else if (cmd == PRIVMSG_CMD && _paramCounter < 2)
        return false;
    else if (cmd == LOGTIME_CMD && _paramCounter < 2)
        return false;
    else if (cmd == LOCATION_CMD && _paramCounter < 2)
        return false;
